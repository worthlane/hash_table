...
Возможно мы сможем улучшить результат, если мы уменьшим размер хэш таблицы. Тогда сумма сможет "завернуться" так, чтобы переполнив размер таблицы, покрыть зазор между пиками. Уменьшим размер таблицы до ~500 и посмотрим результат.
...
Дисперсия увеличилась, но стоит учитывать, что мы уменьшаем размер таблицы, оставляя количество введенных слов тем же. То есть мы очень сильно переполняем хэш-таблицу, но дисперсия сравнительно с длинами списков оказывается достаточно мала.  
