...
Как мы можем заметить, без флагов оптимизации компилятор не смог понять что мы хотим сделать операцию поворота. Давайте теперь добавим флаг ```-O3``` и сравним результаты.

![](assets/pictures/ror_O3.png)

А с оптимизацией компилятор уже догадался заменить функцию одной командой ```ror```. Победа!!!
...
Компилятор вновь не смог заметить требуемую нами команду. Теперь с оптимизацией:

![](assets/pictures/ror_O3.png)

Все аналогично прошлой функции, компилятор может заметить команду поворота только при оптимизации с флагом ```-O3```.
